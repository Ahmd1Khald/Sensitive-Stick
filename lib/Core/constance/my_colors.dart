import 'package:flutter/material.dart';

class MyColors {
  static const Color darkBrown = Color(0xff885718);
  static const Color babyBrown = Color(0xffA85000);
  static const Color lightBrown = Color(0xffFFE9CE);
  static const Color creamColor = Color(0xffFAF6EB);
  static Color yellowColor = const Color(0xffFBC821);
  static Color? grayColor = Colors.grey[400]; //const Color(0xff9094A0);
  static Color? buttonsColor = Colors.blue[300]; //const Color(0xff0df2e0);
  static Color fieldsColor = const Color(0xff696574);
  static Color backGroundColor = const Color(0xff201a31);

  static const Color primaryColor = Color(0xff9B7EF8);
  static const Color orange = Color(0xffEF9A53);
  static const Color babyOrange = Color(0xffF5D5AE);
  static const Color darkBlue = Color(0xff1D2356);
}
